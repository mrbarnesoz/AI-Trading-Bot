"""Parameter sweep for the maker mean-reversion band strategy."""
...
DEFAULT_GRID = {
    "timeframe": ["15m"],
    "band_lookback": [40, 60, 100],
    "z_entry": [1.2, 1.6, 2.0],
    "tp_k_atr": [0.6, 0.9, 1.2],
    "sl_k_atr": [0.8, 1.1, 1.4],
    "signals.post.min_hold_bars": [8, 16, 24],
    "signals.post.hysteresis_k_atr": [0.6, 0.9, 1.2],
    "trailing.trail_k": ["off", 3.0],
    "trailing.ttp_k": ["off", 3.0],
    "risk.cap_fraction": [0.01, 0.02, 0.03],
    "meta.trend.min_strength": [0.0, 0.15],
...
def _passes(summary: Dict[str, float], timeframe: str) -> bool:
    min_trades = ACCEPT["min_trades_1h"] if timeframe.endswith("h") else ACCEPT["min_trades_15m"]
    return (
...
def _configure_iteration(
    ...
    cfg.strategy.name = "maker_reversion_band"
    ...
    cfg.strategy.params.update(
        {
            "band_lookback": int(combo["band_lookback"]),
...
def main(
    ...
    out_csv = out_dir / f"maker_reversion_band_{timestamp}.csv"
    ...
    with out_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        for values in grid_values:
            ...
            logger.debug(
                "Reversion sweep timeframe=%s band=%s z=%.2f tp=%.2f sl=%.2f hold=%s hyst=%.2f trail=%s/%s cap=%.3f spread=%.1f -> Sharpe=%.4f trades=%s",
                combo["timeframe"],
                combo["band_lookback"],
                combo["z_entry"],
                combo["tp_k_atr"],
    ...
    logger.info("Maker reversion-band sweep results written to %s", out_csv)
    return out_csv
