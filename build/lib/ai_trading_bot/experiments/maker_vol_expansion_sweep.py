"""Parameter sweep for the maker volatility expansion strategy."""
...
DEFAULT_GRID = {
    "timeframe": ["1h"],
    "nr_lookback": [4, 7, 14],
    "break_k_atr": [0.8, 1.1, 1.5],
    "confirm_ma_len": [20, 50],
    "signals.post.min_hold_bars": [24, 48],
    "signals.post.hysteresis_k_atr": [0.3, 0.5],
...
def _passes(summary: Dict[str, float], timeframe: str) -> bool:
    if timeframe.endswith("4h"):
        min_trades = ACCEPT["min_trades_4h"]
    elif timeframe.endswith("2h"):
...
def _configure_iteration(base_cfg: AppConfig, combo: Dict[str, object]) -> AppConfig:
    ...
    cfg.strategy.name = "maker_vol_expansion"
    ...
    cfg.strategy.params.update(
        {
            "nr_lookback": int(combo["nr_lookback"]),
            "break_k_atr": float(combo["break_k_atr"]),
            "confirm_ma_len": int(combo["confirm_ma_len"]),
        }
    )
...
def main(
    ...
    out_csv = out_dir / f"maker_vol_expansion_{timestamp}.csv"
    ...
    with out_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        for values in grid_values:
            ...
            logger.debug(
                "Vol expansion sweep timeframe=%s nr=%s break=%.2f confirm=%s hold=%s hyst=%.2f trail=%.2f/%.2f cap=%.3f spread=%.1f -> Sharpe=%.4f trades=%s",
                combo["timeframe"],
                combo["nr_lookback"],
                combo["break_k_atr"],
                combo["confirm_ma_len"],
    ...
    logger.info("Maker volatility expansion sweep results written to %s", out_csv)
    return out_csv
